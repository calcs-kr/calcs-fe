...
function ChartCalnder(props: any) {
    let schedule: any = []

    // 캘린더 데이터 형태 변경
    props?.data.map((item: any) => { schedule = [ ...schedule, ...getDatesStartToLast(item.startAt, item.endAt, item.seq) ] })

    // 시작과 끝을 작성하면 모든 날짜의 데이터 생성
    function getDatesStartToLast(startDate: string, lastDate: string, service: []) {
        var result = []
